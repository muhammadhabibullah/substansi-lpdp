...
7. **Post-v1**: free-tier Cloudflare Worker proxy with shared `gpt-5-mini` key + rate limiting, voice mode (STT/TTS — voice *input* shipped via the browser Speech Recognition API, WhatsApp-style unified composer — pill input whose right icon swaps mic ↔ send, a recording bar (timer + waveform, pause/resume) that shows no live text, then an editable transcript review before sending; TTS pending), report history (shipped — one report per attempt, viewable/deletable history list), question bank enriched from awardee experiences.
